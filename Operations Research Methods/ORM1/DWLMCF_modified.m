function [F, optval, U, y, I] = DWLMCF_modified(capMat, costMat, comVec, n, k, Case, showProgress)
    ...
    while 1
        ...
        while 1
            ...
            if secondFlag == 0
                ...
                % Else go to other half next rotation and break loop
                else
                    if subFlag == 1
                        subFlag = 2;
                    else
...
% U = LocalNaiveSolution(capacities, costs, existingArcs, comVec, n, k)
    ...
    for j=1:10
        ...
        while i <= k
            ...
            path = shortestpath(G, comVec(permind,1), comVec(permind,2));
        
            % If the path is empty, there is no path and go to next commodity
            if isempty(path)
                if demandMet < comVec(permind,3)
                    feasible = 0;
                    break
                end
                i = i + 1;
            ...
            usedArcInd = ismember(existingArcs, activatedArcs);
            capPath = copyCapacities(usedArcInd)';
            flowsize = min([capPath, comVec(permind,3) - demandMet]);
            ...
            corRowInd = find(usedArcInd)';
            rowIndices = [rowIndices, corRowInd];
            columnIndices = [columnIndices, permind * ones(1, pathLength-1)];
