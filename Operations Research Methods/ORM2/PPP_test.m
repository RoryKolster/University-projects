function [d, x, y] = PPP(n, cfun, T, sP)
    ...
    step_sizes = [0.01,0.005,0.001, 0.0001];
    ...
    while toc(Loop_time) < T
        ...
        elseif toc(no_improv) > max_time_no_improv && last_int == 0
            ...
            step_sizes = [0.001,0.0005,0.0001, 0.00005, 0.00001];
        ...
        elseif toc(Loop_time)> 0.9*T && last_int == 0
            ...
            % Change the step_sizes to smaller scale to search more
            % intensively
            step_sizes = [0.002,0.001,0.0005,0.0001];
        end
...
function [min_d, x, y, numbers, directions, Aspiration_flag1, Aspiration_flag2] = neighbourhood(n, x, y, cfun, tabu_list, d_opt, point_directions, step_sizes)
    ...
    for i = 1:2
        if check(i) ==1
            ...
            if size(index,2) == 1
                tabu(1:3,i) = tabu_list(2:4, check(i));
            else
        ...
        for p = 1:num_dir
            for j =1:num_steps
                ...
                elseif ismember(i, tabu(:,1)) == 1  && ismember(p, tabu(:,2)) == 0
                    ...
                    new_xy(counter,5) = 9;
                    new_xy(counter,6) = p;
                    if all(point_directions(p,:) == [0,0])            % at least one point has to move
                        new_xy(counter,:) = []; 
                        continue
                    end
                ...
                elseif ismember(i, tabu(:,1)) == 0  && ismember(p, tabu(:,2)) == 1
                    ...
                    new_xy(counter,5) = i;
                    new_xy(counter,6) = 9;
                    if all(point_directions(i,:) == [0,0])            % at least one point has to move
                        new_xy(counter,:) = []; 
                        continue
                    end
                else
                ...
                elseif cfun(new_xy(counter,3),new_xy(counter,4)) == 0 % Checks if second new point is feasible
                   new_xy(counter,:) = []; 
                   continue
                elseif all(point_directions(i,:) == [0,0]) && all(point_directions(p,:) == [0,0]) % At least one point has to move (i == 9 && p == 9)
                   new_xy(counter,:) = []; 
                   continue
                end
                counter = counter + 1;
    ...
    if isempty(new_xy) && isempty(tabu_xy)
        ...
        if isempty(new_xy) % Bug Handling
            ...
            % good_numbers = (1:1+2*k).*(~ismember(1:1+2*k, 1:3:1+2*k));
            % good_numbers = good_numbers(good_numbers ~= 0);
        ...
        elseif min_d_tabu > d_opt % Aspiration criterion check
            ...
            % Saves the direction in which the two points move
